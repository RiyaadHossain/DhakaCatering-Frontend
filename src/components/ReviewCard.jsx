...
export default function ReviewCard({ review, user }) {
  ...
  const [deleteReview, { isLoading: deleteLoading, isError: deleteError, isSuccess: deleteSucess }] = useDeleteReviewMutation()

  useEffect(() => {
    if(deleteLoading) toast.loading("Deleting...", {id: 'deleteLoad', duration: 800 })
    if(deleteSucess) toast.success("Deleted your Review successfully", {id: 'deleteSucc' })
    if(deleteError) toast.error("Sorry! Cloudn't delete your review", {id: 'deleteErr' })
